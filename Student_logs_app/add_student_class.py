...
class add_student(Toplevel):
    ...
    def save_student(self):
      ...
      c.execute("CREATE TABLE IF NOT EXISTS students(ΟΝΟΜΑ TEXT,ΕΠΙΘΕΤΟ TEXT, ΤΗΛΕΦΩΝΟ TEXT, EMAIL TEXT, ΣΧΟΛΙΑ TEXT)")
      
      c.execute("INSERT INTO students VALUES(?,?,?,?,?)",(name_inf,surname_info,phone_info,email_info,comments_info))
    ...
    def show_error_message(self,error_message,entry_widget):
        ...
        error_label = ttk.Label( error_window, text=error_message, style="Custom.TLabel")
        error_label.pack()
        ok_error_button = ttk.Button( error_window, text="OK", command= lambda: self.close_error_window(error_window,entry_widget), style="Custom.TButton")
        ok_error_button.pack()
    
    def close_error_window(self,error_window,entry_widget):
